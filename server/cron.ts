/*
 Basic backend cron example using node-cron.
 - Reads schedule from env (defaults to 8 PM local: "0 20 * * *")
...
async function dispatchSummary(summary: { date: string; completed: string[]; pending: string[] }) {
  // Replace with actual push/email dispatch
  console.log('[GuiltPing Summary]', summary);
}

function startCron() {
  console.log(`Starting GuiltPing summary cron: ${SCHEDULE}`);
  cron.schedule(SCHEDULE, async () => {
    const summary = aggregateSummary();
    await dispatchSummary(summary);
  }, {
    timezone: process.env.SUMMARY_TIMEZONE || 'Etc/UTC',
  });
}
